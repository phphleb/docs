...
namespace Phphleb\Docs\Deployment;
...
class StartForHleb implements DeploymentLibInterface
{
    ...
    {
        return 'Adding or removing a project with framework documentation.';
    }
    ...
    public function add(): int
    {
        ...
        // Adding a configuration file with documentation page settings.
        // Добавление конфигурационного файла с настройками страниц документации.
        $versions = [20];
        $result = 0;
        foreach($versions as $v) {
            $originPath = Settings::getPath("@library/docs/match-directory/structure/docsv$v.php");
            $targetPath = Settings::getPath("@/config/structure/docsv$v.php");
            $result = $result ?: (new StructureMigration())->add($originPath, $targetPath, $this->quiet, true);
        }
