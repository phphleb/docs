...
<p class="hl-info-block">
    По умолчанию, в режиме <span class="notranslate">DEBUG</span> эта настройка прав не нужна, также на хостингах обычно предоставляются расширенные права, поэтому, если проект находится в режиме разработки или на хостинге, можно пропустить этот шаг.
</p>
...
<?= Paragraph::h2('Автозагрузка классов') ?>

<p>
    Вместе с <span class="notranslate">Composer</span> поставляется универсальный автозагрузчик классов, его использование предпочтительно.
    Если файл(класс) не был найден, будет произведена попытка загрузить его вспомогательным автозагрузчиком фреймворка, который ориентируется на <span class="notranslate">PSR-0</span> именования и работает независимо от Composer.
    Например, для автозагрузчика фреймворка, класс <span class="notranslate">App\Controllers\ExampleController</span> должен соответствовать файлу <span class="notranslate">/app/Controllers/ExampleController.php</span> в проекте.
</p>
