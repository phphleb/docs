...
<p>
    Несмотря на то, что создание объекта в контейнере через new и c пустым конструктором является хорошей практикой, в конце концов вы можете вынести в отдельный метод специального класса создание всех необходимых зависимостей и зарегистрировать его выполнение в контейнере, несмотря на это есть способы
    разрешения зависимостей не прибегая к созданию отдельного класса-обёртки.
</p>
<p>
    В случае возникновения необходимости переиспользовать сервис из контейнера для инициализации другого сервиса в контейнере обратимся к возможностям, которые даёт <a href="<?= Link::url('docs.2.0.container.di.page'); ?>">внедрение зависимостей</a>. В классе App\Bootstrap\ContainerFactory эти методы доступны, как и в отдельном специальном классе для создания контейнера.
</p>
<p>
    Например, необходимо проинициализировать конструктор сервиса в контейнере. Для этого в теле оператора match класса
    App\Bootstrap\ContainerFactory нужно добавить примерно такое соответствие:
</p>
